...
function Project({img, title, github, deploy, children}) {
  return (
    <div className="w-[265px] max-sm:w-96 max-2xl:w-[350px] flex flex-col border shadow-sm rounded-xl bg-black border-gray-700 shadow-slate-700/[.7]">
      ...
      <div className="p-4 md:p-5 h-[148px]">
        ...
        {github && 
          <a className="bg-slate-800 font-medium tracking-wider px-4 py-2 rounded-full hover:bg-slate-900 mr-3"
          target="_blank"
          href={github} rel="noreferrer">
          <i className="fa-brands fa-github"></i> Github</a>
        }
        {deploy && 
          <a className="text-[#F1D827] bg-slate-800 font-medium tracking-wider px-4 py-2 rounded-full hover:bg-slate-900"
          target="_blank"
          href={deploy} rel="noreferrer">
          <i className="fa-solid fa-link font-bold"></i> Live</a>
        }
      </div>
    </div>
  )
